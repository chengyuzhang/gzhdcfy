...
export default {
	...
	async created(){
		...
		this.clinicDate = this.$route.query.clinicDate
	},
	mounted(){
		let _this = this
		let idx = 0
		let timer = setInterval(() => {
			...
			let elWrap = document.querySelector('.list-wrap')
			let elUl = document.querySelector('.list-wrap ul')

			let elItem = elWrap.querySelectorAll('li')[0]
			let elItemWidth = elItem && elItem.getBoundingClientRect().width
			if(elItemWidth){
				elUl.style.width = this.dateList.length * elItemWidth + 'px'

				this.dateList.forEach((item, index) => {
					if(item.date == this.clinicDate){
						idx = index
						_this.changeTab(index, item)
					}
				})
		...
		// 	let elWrap = document.querySelector('.list-wrap')
		// 	let elUl = document.querySelector('.list-wrap ul')
		// 	let elItemWidth = elWrap.querySelectorAll('li')[0].getBoundingClientRect().width
		// 	elUl.style.width = this.dateList.length * elItemWidth + 'px'
